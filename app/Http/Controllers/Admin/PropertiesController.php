...
class PropertiesController extends MainAdminController
{
    ...
    { 
		...
		if(!empty($inputs['id'])){
            ...
            //return \Redirect::back();
            return redirect()->route('payPremium',$property->id);
        }		     
    ...
    { 
		...
		if($property->status==1) {
			...
			if($property->is_paid == 0 ) { //and Auth::User()->usertype!="Admin" 
				\Session::flash('flash_message', 'Please make payment to publish.');
				return redirect()->route('payPremium',$property->id);
			}
			$property->status='1';		 
	   		$property->save();
